...
class MoleculeBare:
    ...
    def compare_densities_fci(self, pass_object: Union[bool, class_qnb.HamiltonianV2] = False,
                              calculate_per_site=False) -> typing.Tuple[
        np.array, class_qnb.HamiltonianV2, tuple, np.array]:
...
class Molecule(MoleculeBare):
    ...
    def find_solution_as_root(self, starting_approximation=None):
        if starting_approximation is None:
            # Get some starting approximation for the optimization algorithm
            starting_approximation = np.zeros(len(self.equiv_site_helper_list) - 1, float)
    ...
    def cost_function_whole(v_hxc_approximation: np.array, mol_obj: 'Molecule') -> np.array:
        ...
        for site_group, group_tuple in enumerate(mol_obj.equiv_atom_groups):
            ...
            if first_iteration:
                ...
                sol = scipy.optimize.root_scalar(mol_obj.cost_function_2,
                                                 args=(
                                                     mol_obj.embedded_mol, h_tilde_dimer, u_0_dimer,
